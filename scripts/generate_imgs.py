...
class S2C_Dataset(Dataset):
    def __init__(self, json_file, mode='simple'):
        self.mode = mode
        # 读取json文件
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'simple':
            sample_item["text"] = sample["simple"]
        elif self.mode == 'complex':
            sample_item["text"] = sample["complex"]
        else:
            raise ValueError("Invalid mode. Use 'simple' or 'complex'.")
...
def generate_images(args):
    ...
    mode = args.img_save_dir.split("/")[-1]
    # 创建自定义数据集
    dataset = S2C_Dataset(args.prompt_json_path, mode = mode)
    print(f"Mode: {mode}")
    ...
    progress_bar = tqdm(range(step_start, step_end - step_start))
    ...
    for step ,batch in enumerate(data_loader):
        ...
        id = batch["id"][0].item()
        img_name = f"{id}.png"
        if img_is_exit(args.img_save_dir, img_name):
        ...
        image = run_inference(g, prompt, device)[0]
        # return image_path_list

        img_save_path = os.path.join(args.img_save_dir, img_name)
...
if __name__ =="__main__":
    ...
    parser.add_argument(
        "--prompt_json_path",
        default="/home/khf/liutao/train_reward/json/complex.json",
        type=str,
        help="Path to the prompts json list file, each item of which is a dict with keys `id` and `prompt`.",
    )
    parser.add_argument(
        "--img_save_dir",
        default="/data/liutao/datasets/s2c_images/complex",
        type=str,
